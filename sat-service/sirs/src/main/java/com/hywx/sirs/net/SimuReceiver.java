...
public class SimuReceiver {
	...
	public SimuReceiver(InetSocketAddress groupAddress) {
		this.socket = createMulticast(groupAddress);
		this.recvBuf = new byte[RECV_BUFFER_SIZE];
	}
	
	public SimuReceiver(InetSocketAddress groupAddress, String local) {
	...
	public void recv() {
		...
		try {
			...
			//System.out.println("*************udp multicast recv: " + packet.getLength() + ", " + ByteBufUtil.hexDump(data));
			
			if (packet.getLength() > FRAME_HEADER_LENGTH) {
				//接收的数据放入队列
				GlobalQueue.getSimuQueue().put(data); 
			}
	...
	public void send(byte[] data, InetSocketAddress groupAddress) {
		...
		try {
			...
			//System.out.println("*************udp multicast send: " + packet.getLength() + ", " + ByteBufUtil.hexDump(data));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
