...
public interface FaultMapper {
	...
	// 查询故障卫星信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 0 ORDER BY SATELLITEID")
	List<FaultSend> listFaultSatellites();
	
	// 根据ID查询故障卫星信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 0 AND SATELLITEID = #{satelliteId} ORDER BY SATELLITEID")
	List<FaultSend> listFaultSatellitesById(@Param("satelliteId") String satelliteId);
		
	// 根据级别查询故障卫星信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 0 AND LEVEL = #{level} ORDER BY SATELLITEID")
	List<FaultSend> listFaultSatellitesByLevel(@Param("level") Integer level);
			
	// 根据ID和级别查询故障卫星信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 0 AND SATELLITEID = #{satelliteId} AND LEVEL = #{level} ORDER BY SATELLITEID")
	List<FaultSend> listFaultSatellitesByIdLevel(@Param("satelliteId") String satelliteId, @Param("level") Integer level);
	...
	// 查询故障信关站信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 1 ORDER BY GROUNDSTATIONID")
	List<FaultSend> listFaultGroundStations();
	
	// 根据ID查询故障信关站信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 1 AND GROUNDSTATIONID = #{groundStationId} ORDER BY GROUNDSTATIONID")
	List<FaultSend> listFaultGroundStationsById(@Param("groundStationId") String groundStationId);
		
	// 根据级别查询故障信关站信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 1 AND LEVEL = #{level} ORDER BY GROUNDSTATIONID")
	List<FaultSend> listFaultGroundStationsByLevel(@Param("level") Integer level);
			
	// 根据ID和级别查询故障信关站信息
	@Select("SELECT TYPE, SATELLITEID, GROUNDSTATIONID, DEVICEID, MAINID, MAINNAME, SUBID, SUBNAME, LEVEL, WAITING FROM T_FAULT_SEND WHERE TYPE = 1 AND GROUNDSTATIONID = #{groundStationId} AND LEVEL = #{level} ORDER BY GROUNDSTATIONID")
	List<FaultSend> listFaultGroundStationsByIdLevel(@Param("groundStationId") String groundStationId, @Param("level") Integer level);
