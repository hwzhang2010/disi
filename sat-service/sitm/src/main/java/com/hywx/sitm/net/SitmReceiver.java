 ...
 *
 */
public class SitmReceiver {
	
	//private static Logger log = LoggerFactory.getLogger(SitmReceiver.class);
	...
	public SitmReceiver(InetSocketAddress groupAddress) {
		this.socket = createMulticast(groupAddress);
		this.recvBuf = new byte[RECV_BUFFER_SIZE];
	}
	
	public SitmReceiver(InetSocketAddress groupAddress, String local) {
	...
	public void recv() {
		...
		try {
			...
			//log.info("*************udp multicast recv: " + packet.getLength() + ", " + ByteUtil.toHex(data));
